...
extern const struct http_resource http_resources[] = {
  ...
  {"images/ewu_bg.png", "image/png"},
  {"images/ewu_checkedfolder.png", "image/png"},
  {"images/ewu_openfolder.png", "image/png"},
  {"images/ewu_quit.png", "image/png"},
  {"images/ewu_welcome.png", "image/png"},
  {"images/ewu_send.png", "image/png"},
  {"images/ewu_ticksuccess.png", "image/png"},
  {"images/ewu_tickwrong.png", "image/png"},
  {"images/ewu_top_bg.png", "image/png"},
  {"erreur_fichier_nul", "text/html"},
  {"succes", "text/html"},
  {"quit", "text/html"},
  {"openRep", "text/html"},
  {"erreur_404", "text/html"},
  {"", ""}
};
...
static int time_to_httpdate(char* http_date);
...
int
http_match_resource(char* resource) {
  int i;

  if (*resource == '/' && *(resource + 1) == '\0')
    return 0;
  else if (*resource == '/' && strcmp(resource + 1, "credits") == 0)
    return 1;
  else if (*resource == '/' && strcmp(resource + 1, "favicon.ico") == 0)
    return 2;

  for (i = 3; http_resources[i].resource[0] != '\0'; i++)
    if (strcmp(resource + 1, http_resources[i].resource) == 0)
      return i;
